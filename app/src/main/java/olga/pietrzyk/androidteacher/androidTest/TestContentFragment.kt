...
class TestContentFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val binding = DataBindingUtil.inflate<FragmentTestContentBinding>(
            inflater,
            R.layout.fragment_test_content, container, false
        )

        val application = requireNotNull(this.activity).application
        val viewModelFactory = TestContentViewModelFactory(application)

        viewModel = ViewModelProviders.of(this, viewModelFactory).get(TestContentViewModel::class.java)
        ...
        binding.answerButton.setOnClickListener {
            ...
            if (idIndex != indexOutOfRange && viewModel.indexOfQuestion < (viewModel.numberOfQuestions - decreaseForQuestionNumber)) {
                ...
                if (viewModel.indexOfQuestion < (viewModel.numberOfQuestions)) {
                    viewModel.questionNumber.observe(
                        viewLifecycleOwner,
                        Observer { newQuestionNumber ->
